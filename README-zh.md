...
- 全屏、窗口状态恢复、重新加载、新窗口副本、打印、快捷键帮助、完整错误页和
  “关于/调试信息”窗口
- 简体中文和英文界面模式
...
导入书库的图书会与封面、元数据、阅读进度、书签和批注一起保存在 IndexedDB。
绿色版中该数据库位于 `Data/WebView2`。使用其他 Windows 程序打开书库图书时，
当前实现会创建临时副本；对于特别大的图书，这个显式操作仍可能产生一次额外的
内存峰值。
...
- 保留 Windows 现有的文件关联和默认应用，不主动修改；
- 按照 Windows 规范，将设置和应用数据保存到当前用户的数据目录。

用户仍可在 Windows 的“打开方式”中手动选择 Foliate，也可以把图书路径作为
启动参数传给程序。安装器不会注册电子书扩展名，也不会静默将 Foliate 设为默认
阅读器。
...
通过窗口选择或拖放的图书会直接作为浏览器 `File` 对象交给阅读核心。通过程序
启动参数传入的路径则由 Tauri 提供类似 Blob 的区间读取接口，使 EPUB 和 PDF
解析器可以按需读取文件片段，而不必先通过 IPC 完整传输一次文件。
