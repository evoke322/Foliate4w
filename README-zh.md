...
- 分开的界面、电子书阅读、PDF 阅读和划词工具设置
- 分页布局下使用鼠标滚轮翻页
- 可整体隐藏划词工具条，并逐项启用或禁用工具
- 剩余时间、章节、出版物页码、Landmarks、EPUB CFI 复制粘贴及进度条章节标记
  导航
...
安装版导入书库时只保存原始 Windows 文件路径，不会再保存一份完整图书副本。
IndexedDB 保存该路径、封面、元数据、阅读进度、书签和批注；绿色版中该数据库
...
- 写入正常的卸载信息；
- 在缺少 WebView2 Runtime 时显示微软交互式引导程序，由用户确认或取消安装；
- 注册受支持格式，使 Foliate 出现在 Windows“打开方式”候选列表；
- 保留 Windows 现有的默认应用，不主动修改；
- 按照 Windows 规范，将设置和应用数据保存到当前用户的数据目录。

用户可在 Windows 的“打开方式”中手动选择 Foliate，也可以把图书路径作为启动
参数传给程序。这里的注册只声明 Foliate 支持这些格式，不会静默将 Foliate 设为
默认阅读器。
...
Runtime。安装版缺少它时会交互式启动引导程序，不再静默安装。为了保持绿色版
自包含且不主动修改系统，绿色版要求系统已经提供 WebView2。
...
7. 生成会注册“打开方式”候选、但不会替换用户默认应用的安装版；
8. 生成数据隔离的绿色版目录；
9. 将绿色版目录压缩为 ZIP；
...
安装版使用 Windows 原生文件选择器，因此打开和导入的图书可以保留原始文件路径。
